...
public class MarkerDetectionUIController : MonoBehaviour, ITangoVideoOverlay
{
    ...
    public GameObject blankcursor;
    public int counter;
    public bool isPainting;

    public BrushManager brushM;
    public dLineManager lineM;
    public PinchDraw cilM;
    ...
    public void Update()
    {
        //code to put back the cursor if not found
        ...
        TangoUnityImageData imageBuffer)
    {
        ...
        if (m_markerList.Count < 2 && isPainting == true )
        {
            lineM.painterEnd();
            cilM.paintEnd();
            brushM.PaintingEnd();
        ...
        for (int i = 0; i < m_markerList.Count; ++i) 
        {
            ...
            if (i == 0)
            {
                ...
                {
                    m_planets[planetIndex] = cursor.gameObject;
                }
            }
            if (i == 1)
            {
                if (m_planets[planetIndex] == null)
                {
                    m_planets[planetIndex] = blankcursor.gameObject;
                }
            }
            if (i == 0)
             {

                if (m_markerList.Count == 1)
                {
